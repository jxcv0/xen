...
static inline vec3_t scalar_vec3(const vec3_t v, float s) {
  vec3_t sv = {.values = {v.values[0] * s, v.values[1] * s, v.values[2] * s}};
  return sv;
}
...
static inline mat4_t ortho(const float left, const float right,
...
static inline mat4_t perspective(const float fov, const float near,
                                 const float far, const float ratio) {
  mat4_t m = {0};
...
static inline mat4_t translate_vec2(const mat4_t m, const vec2_t v,
                                    const float f) {
  mat4_t mat = m;
...
// create view matrix
static inline mat4_t look_at(const vec3_t eye, const vec3_t ctr,
                             const vec3_t up) {
